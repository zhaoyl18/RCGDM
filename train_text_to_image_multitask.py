...
from classifier_free.SD_class_condition import SD_class_condition
...
def log_validation(vae, text_encoder, tokenizer, unet, args, accelerator, weight_dtype, epoch):
    ...
    pipeline = SD_class_condition.from_pretrained(
    ...
    for i in range(len(val_prompts)):
        class_label = torch.tensor(i % 4).to(accelerator.device, dtype=torch.long)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-2,
        help="Initial embed learning rate",
    )
...
def main():
    ...
    param_groups = [
    {
        "params": unet.class_embedding.parameters(),  # Parameters of the class_embedding
        "lr": args.embed_learning_rate
    },
    {
        "params": [param for name, param in unet.named_parameters() if "class_embedding" not in name],  # All other parameters
        "lr": args.learning_rate  # Default learning rate for other parameters
    }
]
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                class_labels = batch["class_labels"].squeeze().to(accelerator.device, dtype=torch.long)
            ...
            if accelerator.sync_gradients:
                ...
                class_embed_norm = torch.norm(accelerator.unwrap_model(unet).class_embedding.weight)
                accelerator.log({"class_embed_norm": class_embed_norm}, step=global_step)
                train_loss = 0.0
